...
export async function POST(req: Request) {
  const t0 = Date.now();
  const tick = (l: string) =>
    console.log(`[contact/submit] ${l} +${Date.now() - t0}ms`);
  ...
  try {
    ...
    try {
      await sendContactNotification(
        process.env.CONTACT_NOTIFY_TO || email, // explicit receiver (you) or fallback to sender
